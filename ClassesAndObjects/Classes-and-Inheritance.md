...
像之前提到的，我们在 kotlin 中坚持做明确的事。不像 java ，kotlin 需要把可以复写的成员都明确注解出来，并且复写它们：
...
对于 `Derived.v()` 来说 override 注解是必须的。如果没有加的话，编译器会提示／如果没有 open 注解，像 `Base.nv()` ,在子类中声明一个同样的函数是不合法的，要么加 override 要门不要复写。在 final 类(就是没有open注解的类)中，open 类型的成员是不允许的。

标记为 override 的成员是 open的，它可以在子类中被复写。如果你不想被复写就要加 final:
...
有个问题就是如何复写子类中那些作者不想被复写的类，下面介绍一些令人讨厌的方案。
